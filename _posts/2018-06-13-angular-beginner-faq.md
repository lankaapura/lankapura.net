---
title: Angular Beginner FAQ
header: null
og_image: null
published: true
---

So you have been assigned to an Angular project? You need to get yourself up to date fast. Here are few questions that I asked myself during my Angular learning journey. Hope this will answer some of your questions.

### Where do I start?

- [Getting started](https://angular.io/guide/quickstart)
- [Learn how to create your first Angular app in 20 minutes](https://medium.freecodecamp.org/learn-how-to-create-your-first-angular-app-in-20-min-146201d9b5a7)

### What are all these tools?
- [Getting Started With Node.js](https://codeburst.io/getting-started-with-node-js-a-beginners-guide-b03e25bca71b)
  
### What is RXJS and Observables
- [A Beginner’s Adventure with RxJS: Part I](https://medium.com/defmethod-works/implementing-rxjs-part-i-daaef12ec1c6)
- [A Beginner’s Adventure with RxJS: Part II](https://medium.com/defmethod-works/a-beginners-adventure-with-rxjs-part-ii-f406898993ef)
- [RXJS](https://www.learnrxjs.io/)
- [RxJS best practices in Angular](https://blog.strongbrew.io/rxjs-best-practices-in-angular/)

### How do I interact with DOM?
- [Exploring Angular DOM manipulation](https://blog.angularindepth.com/exploring-angular-dom-abstractions-80b3ebcfc02)
- [Working with DOM in Angular](https://blog.angularindepth.com/working-with-dom-in-angular-unexpected-consequences-and-optimization-techniques-682ac09f6866)

### What is Angular CLI?
- [Angular CLI](https://angular.io/cli)
- [Angular CLI Guide](http://www.projectcodify.com/angular-cli-guide)

### How do I debug?
- [A Guide To Debugging Angular Applications](https://medium.com/front-end-hacking/a-guide-to-debugging-angular-applications-5a36bd88b4cf)
- [Everything you need to know about debugging Angular applications](https://blog.angularindepth.com/everything-you-need-to-know-about-debugging-angular-applications-d308ed8a51b4)

### Where can I find plugins?
- [A curated list of awesome Angular resources](https://github.com/gdi2290/awesome-angular)
- [ngmodules](http://ngmodules.org/)

### What is SSR?
- [Angular Universal](https://angular.io/guide/universal)
- [Angular Universal & Server-side rendering Deep-Dive](https://medium.com/@MarkPieszak/angular-universal-server-side-rendering-deep-dive-dc442a6be7b7)

### What is AoT?
- [The Ahead-of-Time (AOT) compiler](https://angular.io/guide/aot-compiler)
- [Angular: Writing AoT-friendly applications](https://medium.com/spektrakel-blog/angular-writing-aot-friendly-applications-7b64c8afbe3f)
- [going production ready with Ahead-of-Time (AoT) compilation](https://medium.com/spektrakel-blog/angular2-going-production-ready-with-ahead-of-time-compilation-aot-developer-experiences-aebaa0228909)

### What are the best practices?
- [Style Guide](https://github.com/johnpapa/angular-styleguide)
- [Angular Performance Checklist](https://github.com/mgechev/angular-performance-checklist)

### Angular 1, 2, 4 or 6?
- [Angular Update](https://update.angular.io/)
- [Keeping your Angular CLI project up to date](https://coryrylan.com/blog/keeping-your-angular-cli-project-up-to-date)

### How do I keep myself up to date?
- [Angular Blog](https://blog.angular.io/)
- [swissangular](https://swissangular.com/)
- [angularindepth](https://blog.angularindepth.com/)

